...
def evaluate(sess, args, vocab, model, x, y, result_path=None):
    probs = []
...
def test(sess, args, vocab, model, x, y, result_path=None):
    probs = []
    batches = get_batches(x, y, vocab.word2id, args.batch_size)
    for batch in batches:
        p = sess.run(model.probs,
    ...
    if result_path:
        with open(result_path + '.1', 'w') as f1, open(result_path + '.0', 'w') as f2:
            for i in range(len(same)):
                if same[i]:
                    f1.write(x[i])
                else:
                    f2.write(x[i])
        print("Test result save at: %s and %s" % (result_path + '.1', result_path + '.0'))
...
if __name__ == '__main__':
    ...
    if args.test:
        test_x, test_y = prepare_test(args.test)
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.test:
            test(sess, args, vocab, model, test_x, test_y, result_path=args.output)
