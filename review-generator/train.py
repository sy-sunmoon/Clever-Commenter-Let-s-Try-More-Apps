...
def train_model(model, data, optim, epoch, params):
	...
	for src1, src2, tgt, src_len1, src_len2, tgt_len, \
		original_src1, original_src2, original_tgt in trainloader:
		...
		try:
			loss, outputs = model(src1, src_len1, src2, src_len2, dec, targets)
			pred = outputs.max(2)[1] # pred: [max_tgt_len, batch_size]
			targets = targets.t() # targets: [max_tgt_len, batch_size]
...
def eval_model(model, data, params):
	...
	for src1, src2, tgt, src_len1, src_len2, tgt_len, original_src1, original_src2, original_tgt in validloader:
